...
- The XML file `*.vcxitems.filter` or `*.vcxproj.filter` organize source files in solution explorer folders (virtual folders) that could be different from physical file system, which create a human friendly view.
...
- When adding a source file to a specific solution explorer folder:
  - It must be also added to one or muliple project files.
  - Find the `*.vcxitems.filter` or `*.vcxproj.filters` file with the same name.
  ...
  - Edit that `*.vcxitems.filter` or `*.vcxproj.filters` file to include the source file.

## Renaming and Removing Source Files

- All affected `*.vcxitems`, `*.vcxproj`, `*.vcxitems.filter` and `*.vcxproj.filter` must be updated.
