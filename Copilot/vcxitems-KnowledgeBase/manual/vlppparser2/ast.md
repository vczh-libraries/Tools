...
```AST enum Season { Spring, Summer, Autumn, Winter, } ```

## Class

```AST class Base { var name : token; } class Derived : Base { var season : Season; var baseArray : Base[]; } ```

Inheriting between classes works as other programming languages.

Here are a list of supported field types: - **token**: vl::glr::ParsingToken - **enum type** - **class type**: vl::Ptr\<ClassType\> - **array to class type**: vl::collections::List\<vl::Ptr\<ClassType\>\>
